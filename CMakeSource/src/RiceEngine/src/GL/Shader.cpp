...
Shader::Shader(pGraphicsManager g_mgr) : GraphicsComponentBase(g_mgr) {
	...
	on_resize_uuid = graphics_mgr->resizeEvent.subscribe([this](Vector2i win){ onResize(win); }); // @suppress("Invalid arguments")
}
...
void Shader::setActive() {
	get_api_data().mainCommandBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics, api_data->pipeline);
}
NSP_GL_END
