...
API_Data::API_Data(pGraphicsManager mgr) {
	...
	if(!SDL_Vulkan_CreateSurface(mgr->window->getHandle().get(), instance, (VkSurfaceKHR*)&surface)){
...
void API_Data::init_commands() {
	...
	vk::CommandBufferAllocateInfo cmdAllocInfo = VulkanHelper::command_buffer_allocate_info(commandPool);


	mainCommandBuffer = device.allocateCommandBuffers(cmdAllocInfo)[0];
...
void API_Data::init_def_renderpass(){
...
void API_Data::init_framebuffers() {
...
void API_Data::init_sync_structures() {
...
void API_Data::recreateSwapchain() {
	...
	init_def_renderpass();
	init_framebuffers();
	g_mgr->resizeEvent.invoke({ windowExcent.width, windowExcent.height });
}

void API_Data::cleanupSwapchain(bool destroy) {
...
bool API_Data::beginDraw() {
	...
	res = device.acquireNextImageKHR(swapchain, 1000000000,
	...
	Vector3f flash = { abs(sin(g_mgr->frameNumber / 51.f)), // @suppress("Invalid arguments")
	abs(sin(g_mgr->frameNumber / 43.f)), // @suppress("Invalid arguments")
	abs(sin(g_mgr->frameNumber / 13.f)), // @suppress("Invalid arguments")
			};
	...
	rpInfo.renderPass = def_renderPass;
	...
	mainCommandBuffer.beginRenderPass(&rpInfo, vk::SubpassContents::eInline);

	return true;
}

void API_Data::draw(uint count) {
	mainCommandBuffer.draw(count, 1, 0, 0);
}

void API_Data::endDraw() {
	...
	auto res = vkQueueSubmit((VkQueue) graphicsQueue, 1,
	...
	res = vkQueuePresentKHR((VkQueue) graphicsQueue,
