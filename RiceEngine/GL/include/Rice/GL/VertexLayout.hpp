...
template <typename T> class VertexListT : public VertexList {
  private:
    vec<T> vertices;
  ...
  public:
    ...
    VertexLayout getLayout() const override { return T::vertexLayout; }
    void *getData(uint i) const override { return (void *)&vertices[i]; }

    T &getVertex(uint i) { return vertices[i]; }

    void addVertex(T v) { vertices.push_back(v); }

    void addVertices(VertexListT<T> v) {
        vertices.insert(vertices.end(), v.vertices.begin(), v.vertices.end());
    }

    uint count() const override { return vertices.size(); }
};
...
struct VertexInput {
    ...
    VertexInput(const char *desc, uint binding, uint offset, Format format)
        : description(desc), binding(binding), offset(offset), format(format) {}
};
