...
UniformBuffer_API_Data::UniformBuffer_API_Data(GraphicsManager_API_data& api_data, vk::Device& dev, vk::PhysicalDevice& gpu) : device(dev), GPU(gpu) {
...
void UniformBuffer_API_Data::allocateDescriptorSets(vk::DescriptorSetLayout layout, vk::PipelineLayout pipeline_layout) {
...
UniformBuffer_API_Data& UniformBuffer_API_Data::allocate(size_t size) {
	...
	if(!allocated) {
		for(uint i = 0; i < buffer_count; i++) {
			...
			MemoryRequirements memReq = device.getBufferMemoryRequirements(buffers[i]);
...
UniformBuffer_API_Data& UniformBuffer_API_Data::setData(uint i, void* pData, size_t nData, size_t offset) {
	void* mappedData;
...
UniformBuffer_API_Data& UniformBuffer_API_Data::getData(uint i, void* pData, size_t nData, size_t offset) {
	void* mappedData;
...
uint UniformBuffer_API_Data::findMemoryType(uint typeFilter, vk::MemoryPropertyFlags properties) {
	using namespace vk;
